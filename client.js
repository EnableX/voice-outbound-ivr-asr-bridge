...
function voiceEventHandler(voiceEvent) {
  ...
  if (voiceEvent.playstate !== undefined) {
    if (voiceEvent.playstate === 'playfinished' && voiceEvent.prompt_ref === 'welcome_prompt') {
       ...
       ivrVoiceCall(voiceEvent.voice_id, ttsPlayVoice,'english_us', "Please let me know if you want to connect to sales team or service team", 'voice_menu',true, () => {});
    } else if (voiceEvent.playstate === 'speech_recognized' && voiceEvent.reason === 'RecognizedSpeech' && voiceEvent.text.includes('Sales team')){
       console.log ("["+callVoiceId+"]  menutimeout received");
       ivrVoiceCall(voiceEvent.voice_id, ttsPlayVoice,'english_us', "Thanks you will be connected to sales team", 'voice_menu',true, () => {});
       setTimeout(()=>{bridgeCall(callVoiceId, process.env.FROM , '91xxxxxxxxxx', () => {})},15000);
    } else if (voiceEvent.playstate === 'speech_recognized' && voiceEvent.reason === 'RecognizedSpeech' && voiceEvent.text.includes('Service team')){
       console.log ("["+callVoiceId+"]  menutimeout received");
       ivrVoiceCall(voiceEvent.voice_id, ttsPlayVoice,'english_us', "Thanks you will be connected to service team", 'voice_menu',true, () => {});
       setTimeout(()=>{bridgeCall(callVoiceId, process.env.FROM , '91xxxxxxxxxx', () => {})},15000);
      ...
      if(voiceEvent.digit === '1') {
        ...
        logger.info(`[${callVoiceId}] ${eventMsg}`);
        sseMsg.push(eventMsg);
        ivrVoiceCall(callVoiceId, ttsPlayVoice,'english_us', "Your call will be forwarded to Sales Team", 'voice_menu', false, () => {});
        ...
        logger.info(`[${callVoiceId}] ${eventMsg}`);
        sseMsg.push(eventMsg);
        ivrVoiceCall(callVoiceId, ttsPlayVoice, 'english_us', "Your call will be forwarded to Service Team", 'voice_menu',false, () => {});
        setTimeout(()=>{bridgeCall(callVoiceId, process.env.FROM , numberArray[voiceEvent.digit], () => {})},3000);
      } else if(voiceEvent.digit === '3') {
        const eventMsg = 'Received Digits : ' + voiceEvent.digit + ' Your call will be disconnected';
        logger.info(`[${callVoiceId}] ${eventMsg}`);
        sseMsg.push(eventMsg);
        ivrVoiceCall(callVoiceId, ttsPlayVoice, 'english_us',"Thank you ! Have a nice day", 'voice_menu',false, () => {});
        ...
        if(voiceEvent.digit.length > 0)
          sseMsg.push(eventMsg);
          dtmf_received = false;
          setTimeout(()=>{if(dtmf_received === false) ivrVoiceCall(callVoiceId, ttsPlayVoice,'english_us', "Sorry your have entered wrong digits", 'voice_menu', () => {});} , 1000);
      }
